...
class CNNModel(nn.Module):
    ...
    def __init__(self):
        ...
        self.net3 = nn.Sequential(nn.Conv2d(147,128,1, stride=1, padding=0),
                                  nn.MaxPool2d(1,stride=1, padding=0),
                                  nn.Dropout(0.2),
                                   nn.GELU())

        a1 = [at1.Att(128)]+[res.ResnetBlock(1) for i in range(4)]+[nn.Conv2d(128,256,1, stride=1, padding=0)]+[at1.Att(256)]+[res.ResnetBlock(2) for i in range(44)]

        self.block = nn.Sequential(*a1)


        self.pr = nn.Sequential(nn.Flatten(),
                      nn.Linear(256*12*9,1024),
                        nn.Linear(1024,235))
