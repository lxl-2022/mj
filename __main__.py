...
def obs2response(model, obs):
    logits = model({'is_training': False, 'obs': {'observation': torch.from_numpy(np.expand_dims(obs['observation'], 0)), 'mask': torch.from_numpy(np.expand_dims(obs['action_mask'], 0))}})
