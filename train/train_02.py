...
if __name__ == '__main__':
    ...
    for e in range(2):
        ...
        if os.path.isfile(logdir + 'checkpoint/acc2.npy'):
            all_acc = np.load(logdir + 'checkpoint/acc2.npy')
            np_acc = np.append(all_acc, np_acc)
        np.save(logdir + 'checkpoint/acc2.npy', np_acc)
        if os.path.isfile(logdir + 'checkpoint/acc_t2.npy'):
            all_acc = np.load(logdir + 'checkpoint/acc_t2.npy')
            np_t_acc = np.append(all_acc, np_t_acc)
        np.save(logdir + 'checkpoint/acc_t2.npy', np_t_acc)

        if os.path.isfile(logdir + 'checkpoint/loss2.npy'):
            all_loss = np.load(logdir + 'checkpoint/loss2.npy')
            np_loss = np.append(all_loss, np_loss)
        np.save(logdir + 'checkpoint/loss2.npy', np_loss)

        checkpoint = {"model_state_dict": model.state_dict(),
                        "optimizer_state_dict": optimizer.state_dict(),
                        "epoch": epoch,
                        "acc": acc}
        path_checkpoint = logdir + 'checkpoint/model2.pkl'
