...
if __name__ == '__main__':
    ...
    logdir = '/mnt/storage/lxl/bot/model/'
    ...
    if resume:
        if os.path.isfile(logdir + 'checkpoint/model1.pkl'):
            print("----------LOAD----------")
            checkpoint = torch.load(logdir + 'checkpoint/model1.pkl')
    ...
    for e in range(2):
        ...
        if os.path.isfile(logdir + 'checkpoint/acc1.npy'):
            all_acc = np.load(logdir + 'checkpoint/acc1.npy')
            np_acc = np.append(all_acc, np_acc)
        np.save(logdir + 'checkpoint/acc1.npy', np_acc)
        if os.path.isfile(logdir + 'checkpoint/acc_t1.npy'):
            all_acc = np.load(logdir + 'checkpoint/acc_t1.npy')
            np_t_acc = np.append(all_acc, np_t_acc)
        np.save(logdir + 'checkpoint/acc_t1.npy', np_t_acc)

        if os.path.isfile(logdir + 'checkpoint/loss1.npy'):
            all_loss = np.load(logdir + 'checkpoint/loss1.npy')
            np_loss = np.append(all_loss, np_loss)
        np.save(logdir + 'checkpoint/loss1.npy', np_loss)

        checkpoint = {"model_state_dict": model.state_dict(),
                        "optimizer_state_dict": optimizer.state_dict(),
                        "epoch": epoch,
                        "acc": acc}
        path_checkpoint = logdir + 'checkpoint/model1.pkl'
