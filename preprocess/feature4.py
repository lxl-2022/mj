...
class FeatureAgent4(MahjongGBAgent):
    ...
    OBS_SIZE = 147  # 圈风门风 2+手牌 4  + 已经出现的牌 4 + 玩家吃牌 4 * 4 + 碰 4 * 1 + 杠 4*1 + 暗杠 1 * 出牌历史4*28 = 147
    ACT_SIZE = 235
    ...
    def __init__(self, seatWind):
        ...
        self.step = np.zeros(4).astype(np.int8)
    ...
    def request2obs(self, request):
        ...
        if t[2] == 'Play':
            ...
            self.his_update(self.step[p],self.curTile)
            self.step[p] += 1
            if p == 0:
                ...
                if not self.wallLast:
                    ...
                    if p == 3 and color in 'WTB':
                        ...
                        if tmp[0] in self.hand and tmp[1] in self.hand:
                            self.valid.append(self.OFFSET_ACT['Chi'] + 'WBT'.index(color) * 21 + (num - 3) * 3 + 2)
                        if tmp[1] in self.hand and tmp[3] in self.hand:
                            self.valid.append(self.OFFSET_ACT['Chi'] + 'WBT'.index(color) * 21 + (num - 2) * 3 + 1)
                        if tmp[3] in self.hand and tmp[4] in self.hand:
                            self.valid.append(self.OFFSET_ACT['Chi'] + 'WBT'.index(color) * 21 + (num - 1) * 3)
        ...
        if t[2] == 'Peng':
            ...
            self.wallLast = self.tileWall[(p + 1) % 4] == 0
            self.peng_update(p,self.curTile)
            if p == 0:
                # Available: Play
                self.valid = []
                for i in range(2):
        ...
        if t[2] == 'Gang':
            ...
            self.gang_update(p,self.curTile)
            if p == 0:
                for i in range(3):
                    self.hand.remove(self.curTile)
                self._hand_embedding_update()
    ...
    def response2action(self, response):
        ...
        if t[0] == 'Chi': return self.OFFSET_ACT['Chi'] + 'WBT'.index(t[1][0]) * 7 * 3 + (int(t[2][1]) - 2) * 3 + int(t[1][1]) - int(t[2][1]) + 1
    ...
    def _obs(self):
        ...
        return {
            ...
            'action_mask': mask,
            'titleWall': self.tileWall.copy()
        }
    
    def _hand_embedding_update(self):
        self.obs[self.OFFSET_OBS['HAND'] : self.OFFSET_OBS['SHOW_TILE']] = 0
        d = defaultdict(int)
    ...
    def chi_update(self, player, tile):
        player = int(player)
        n = 1
        for i in range(1,5) :
            if self.obs[self.OFFSET_OBS['CHI'] + player * 4 + i,self.OFFSET_TILE[tile]] == 1:
                n += 1
        for i in range(1,5):
            self.obs[self.OFFSET_OBS['CHI'] + player * 4 + i,self.OFFSET_TILE[tile]] = 1
        n = 0
        tile = self.OFFSET_TILE[tile]-2
        for i in range(3):
            tile += 1

            for u in self.obs[self.OFFSET_OBS['SHOW_TILE'] :self.OFFSET_OBS['SHOW_TILE']+4 ,tile]:
                if u == 1:
                    n += 1

            self.obs[self.OFFSET_OBS['SHOW_TILE']:self.OFFSET_OBS['SHOW_TILE'] + n+1, tile] = 1

    def peng_update(self,p,tile):

        self.obs[self.OFFSET_OBS['PENG']+p][self.OFFSET_TILE[tile]] = 1

        n = 0
        for u in self.obs[self.OFFSET_OBS['SHOW_TILE']:self.OFFSET_OBS['SHOW_TILE'] + 3+1, self.OFFSET_TILE[tile]]:
            if u == 1:
                n += 1
        n += 2

        self.obs[self.OFFSET_OBS['SHOW_TILE']:self.OFFSET_OBS['SHOW_TILE'] + n + 1, self.OFFSET_TILE[tile]] = 1

    def gang_update(self, p,tile):
        self.obs[self.OFFSET_OBS['GANG']+p][self.OFFSET_TILE[tile]] = 1
        n = 4

        self.obs[self.OFFSET_OBS['SHOW_TILE']:self.OFFSET_OBS['SHOW_TILE'] + n + 1, self.OFFSET_TILE[tile]] = 1

    def angang_update(self,tile):
    ...
    def his_update(self,step,tile):
        if step > 28:
            step = 28
        step = step
        self.obs[self.OFFSET_OBS['HISTORY']: self.OFFSET_OBS['HISTORY'] + step, self.OFFSET_TILE[tile]] = 1
