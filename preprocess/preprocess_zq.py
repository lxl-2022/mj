from feature1 import FeatureAgent1
from feature2 import FeatureAgent2
from feature3 import FeatureAgent3
from feature4 import FeatureAgent4
from feature5 import FeatureAgent5
from feature6 import FeatureAgent6
import numpy as np
import json
import os

FeatureAgent = [FeatureAgent1,FeatureAgent2,FeatureAgent3,FeatureAgent4,FeatureAgent5,FeatureAgent6]
a_1 = [[0,1,2,3],[1,0,2,3],[1,2,0,3],[0,2,1,3],[2,0,1,3],[2,1,0,3]]
logdir = r'C:/Users/lxl/Desktop/dlw/1/data/'
...
def saveData():
    ...
    np.savez(logdir +'data/%d.npz'%matchid
...
matchid = -1
l = []
if os.path.exists(logdir + 'data'):
    pass
else:
    os.mkdir(logdir + 'data')
zq = 0
with open(logdir+'/data_train.txt',encoding='ISO-8859-1') as f:  #'ISO-8859-1'
    line = f.readline()
    while line:
        t = line.split()
        if len(t) == 0:
        ...
        if t[0] == 'Match':
            Dataclear()
            agents = [FeatureAgent[zq](i) for i in range(4)]
            # step_num = -5    五步之后开始算
            if matchid %256 ==0:
...
with open(logdir+'/data/count.json', 'w') as f:
    json.dump(l, f)
