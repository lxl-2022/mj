...
class FeatureAgent5(MahjongGBAgent):
                ...
                if not self.wallLast:
                    ...
                    if p == 3 and color in 'WTB':
                        ...
                        if tmp[0] in self.hand and tmp[1] in self.hand:
                            self.valid.append(self.OFFSET_ACT['Chi'] + 'BWT'.index(color) * 21 + (num - 3) * 3 + 2)
                        if tmp[1] in self.hand and tmp[3] in self.hand:
                            self.valid.append(self.OFFSET_ACT['Chi'] + 'BWT'.index(color) * 21 + (num - 2) * 3 + 1)
                        if tmp[3] in self.hand and tmp[4] in self.hand:
                            self.valid.append(self.OFFSET_ACT['Chi'] + 'BWT'.index(color) * 21 + (num - 1) * 3)
    ...
    def response2action(self, response):
        ...
        if t[0] == 'Chi': return self.OFFSET_ACT['Chi'] + 'BWT'.index(t[1][0]) * 7 * 3 + (int(t[2][1]) - 2) * 3 + int(t[1][1]) - int(t[2][1]) + 1
