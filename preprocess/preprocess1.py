...
def saveData():
    ...
    np.savez(logdir + 'data_v/%d.npz'%matchid
...
with open(logdir + 'data_val.txt', encoding='UTF-8') as f:
    line = f.readline()
    while line:
        t = line.split()
        if len(t) == 0:
            ...
            continue
        if t[0] == 'Match':
            agents = [FeatureAgent1(i) for i in range(4)]
            matchid += 1
            if matchid % 128 == 0:
            ...
            saveData()
        line = f.readline()
with open(logdir +'data_v/count.json', 'w') as f:
    json.dump(l, f)